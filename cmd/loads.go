...
func generateSVG(loads config.LoadsConfig, outputFile string) error {
	...
	for _, load := range allLoads {
		...
		if load.Driver.GetDriverType() == config.ProcessDriverType {
...
func convertDotToSVG(dotString string, outFile string) error {
	if _, err := exec.LookPath("dot"); err != nil {
		return fmt.Errorf("graphviz 'dot' command not found. Please install graphviz (brew install graphviz on macOS)")
	}
